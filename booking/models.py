...
class Appointment(models.Model):
    customer = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, related_name='appointments', null=True, blank=True)
    customer_name = models.CharField(max_length=100, blank=True)
    phone_number = models.CharField(max_length=20, blank=True)
    service = models.ForeignKey(Service, on_delete=models.CASCADE)
    technician = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='assigned_appointments', limit_choices_to={'role': 'technician'}, null=True, blank=True)
    ...
    def __str__(self):
        ...
        return f"{name} - {self.slot}"

    def save(self, *args, **kwargs):
        existing = Appointment.objects.filter(slot=self.slot).exclude(pk=self.pk).count()
